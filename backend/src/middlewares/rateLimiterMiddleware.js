...
export const globalRateLimiter = (maxRequests = 1000, windowMinutes = 15) => {
  const isDev = process.env.NODE_ENV !== "production";
  return createLimiter(
    maxRequests,
    windowMinutes,
    isDev ? (req) => req.path === "/api/health" : undefined
  );
};

// Reusable factories for specific routes
export const createRateLimiter = (maxRequests = 100, windowMinutes = 15, skipCondition) =>
  createLimiter(maxRequests, windowMinutes, skipCondition);

export const createStrictRateLimiter = (maxRequests = 10, windowMinutes = 15) =>
  createLimiter(maxRequests, windowMinutes);
